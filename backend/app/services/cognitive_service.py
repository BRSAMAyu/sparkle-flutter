...
class CognitiveService:
    ...
    async def create_fragment(
        ...
        fragment = CognitiveFragment(
            ...
            severity=severity,
            task_id=task_id,
            created_at=datetime.utcnow() # Note: Use timezone aware in production
        )
    ...
    async def analyze_behavior(self, user_id: UUID, fragment_id: UUID) -> Dict:
        ...
        if fragment.embedding is not None:
            ...
            # Note: casting may be needed depending on DB, but SQLAlchemy pgvector handles it usually
            rag_query = (
                select(CognitiveFragment)
                .where(CognitiveFragment.user_id == user_id)
                .where(CognitiveFragment.id != fragment_id) # Exclude self
        ...
        Task:
        1. Identify the Root Cause (Why did this happen? Deep psychological or habit-based reason).
        2. Identify if this is part of a Pattern (e.g. Planning Fallacy, Perfectionism).
        3. Suggest a SMART Intervention (Specific, Measurable, Actionable, Relevant, Time-bound).
        4. Provide a Confidence Score (0.0 - 1.0) for this pattern recognition.
        ...
        try:
            ...
            logger.error(f"Failed to parse LLM analysis: {response_text}")
